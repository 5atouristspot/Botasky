...
function modify_gunicorn_conf_bind()
{
    ...
    sed -i "s#$before_str.*#$after_str#" ./config/boird_gunicorn_conf.py

}

function run_gunicorn()
{
    nohup gunicorn -c config/boird_gunicorn_conf.py run:app &
}

function sigterm_gunicorn()
{
    guni_pid_file="boird.pid"
